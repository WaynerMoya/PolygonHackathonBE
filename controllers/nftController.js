...
const abiDonatyNft = require("../assets/json/abiDonatyNft.json");
...
const nftController = {
  ...
  getNewestNFTs: async (req, res) => {
    try {
      ...
      for (let i = 0; i < resultNewestNFTsAndCauses.length; i++) {
        ...
        resultNewestNFTsAndCauses[i].address =
          resultNewestNFTsAndCauses[i].nftContract;

        resultNewestNFTsAndCauses[i].logo_foundation =
          resultNewestNFTsAndCauses[i].ethAddress2[0].image;
        resultNewestNFTsAndCauses[i].name_foundation =
          resultNewestNFTsAndCauses[i].ethAddress2[0].name;
        ...
        if (market) {
          resultNewestNFTsAndCauses[i].marketAddress =
            market.get("marketplaceAddress");
        }
        ...
        const status = await contract.callStatic.getListing(
          resultNewestNFTsAndCauses[i].address,
          resultNewestNFTsAndCauses[i].tokenId
        );
        ...
        if (
          ...
          resultNewestNFTsAndCauses[i].price = parseFloat(
            Moralis.Units.FromWei(status["price"])
          );
        }
      }
  ...
  getNFTsFromCause: async (req, res) => {
    try {
      ...
      const market = await consultTable("CauseMarket", address);
  ...
  getNFTsFromAddressAndTokenId: async (req, res) => {
    try {
      ...
      if (results) {
        results.owned = `${results.owner.slice(0, 2)}...${results.owner.slice(
          results.owner.length - 4
        )}`;
        ...
        const market = await consultTable("CauseMarket", results.nftContract);

        if (market) {
          results.marketAddress = market.get("marketplaceAddress");
        }
        const contract = new ethers.Contract(
          results.marketAddress,
          abiMarketPlace,
          signer
        );
  ...
  getNFTsFromWallet: async (req, res) => {
    try {
      ...
      for (let i = 0; i < resultNewestNFTsAndCauses.length; i++) {
        ...
        const contractApproved = new ethers.Contract(
          resultNewestNFTsAndCauses[i].nftContract,
          abiDonatyNft,
          signer
        );

        const getApproved = await contractApproved.callStatic.getApproved(
          resultNewestNFTsAndCauses[i].tokenId
        );

        resultNewestNFTsAndCauses[i].isApproved = false;
        resultNewestNFTsAndCauses[i].status = false;
        if (getApproved !== "0x0000000000000000000000000000000000000000") {
          resultNewestNFTsAndCauses[i].isApproved = true;
